        ...
        try:
            ...
            for x in data["response"]:
                    ...
                    if league_id in leagues_to_follow:
                        ...
                        InsertIntoSql(query,conn)
                        #print("\n"+s_score_home+"\n"+ s_score_away)
                        query=f"INSERT INTO team ( id_team, id_league, id_country, name, logo )values( {teams_home_id}, {league_id}, {country_id}, '{teams_home_name}', '{teams_home_logo}')"
                        InsertIntoSql(query,conn)
                        query=f"INSERT INTO team ( id_team, id_league, id_country, name, logo )values( {teams_away_id}, {league_id}, {country_id}, '{teams_away_name}', '{teams_away_logo}')"
                        InsertIntoSql(query,conn)
